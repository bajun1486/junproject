...
function CampusBoardEdit() {
    ...
    const handleSubmit = async (e) => {
        ...
        data.append('campusBoardFormDto', new Blob([JSON.stringify({
            id: boardId, // boardId를 추가
            title: formData.title,
            content: formData.content
        })], { type: 'application/json' }));
    
        // idList가 필요한 경우 추가 (여기서 필요한 ID 리스트를 설정하세요)
        ...
        try {
            ...
            alert("게시글이 수정되었습니다.");
            navigate('/CampusBoard'); // 수정 후 목록 페이지로 이동 (수정된 경로)
        } catch (error) {
    ...
    return (
        <div className="board-edit-container">
            ...
            <form onSubmit={handleSubmit} className="board-form">
                ...
                <button type="button" className="btn-cancel" onClick={() => navigate('/CampusBoard')}>취소</button>
            </form>
        </div>
    );
}
